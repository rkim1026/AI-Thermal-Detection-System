...
ECE140_MQTT mqtt(MQTT_CLIENT_ID, "ece140a/ta7");
ECE140_WIFI wifi;
...
void setup() {
    ...
    mqtt.subscribeTopic("autograder"); 
...
void loop() {
    ...
    if (takeSingleReading || continuousMode) {
        
        
        takeSingleReading = false; 
        ...
        json += "\"prediction\":\"" + String(present ? "present" : "empty") + "\",";
        json += "\"confidence\":" + String(confidence, 4) + ",";
        json += "\"thermistor\":" + String(pixels[35], 2) + ","; 
        ...
        if (mqtt.publishMessage("autograder", json.c_str())) {
            Serial.printf("[%s] conf=%.3f | MQTT Publish Success\n", 
                          present ? "PRESENT" : "EMPTY  ", confidence);
        } else {
